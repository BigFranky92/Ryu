...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Start to green thread to monitor traffic and calculating free 
        # bandwidth of links respectively.
    ...
    def get_min_bw_of_links(self, graph, path, min_bw):
        """
            ...
            of links is the bandwith, because it is the bottleneck of path.
        """
        _len = len(path)
        if _len > 1:
    ...
    def _get_free_bw(self, capacity, speed):
        # BW:Mbit/s
        return max(float(capacity) - speed * 8/10**6, 0)
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
            ...
            # Get flow's speed.
            pre = 0
            period = settings.MONITOR_PERIOD
	...
	print type
        if(type == 'flow'):
            ...
            print '\n'
        if(type == 'port'):
            print('    datapath       port ''  rx-pkts rx-bytes rx-error ' 'tx-pkts  tx-bytes tx-error port-rate(B/s)''   current-capacity(Mbps)' ' port-stat    link-stat')
            print('---------------- -------- ''------- -------- -------- ' '-------- -------- -------- ' '---------------- ---------------------- ' '----------- -----------')
            format = '%016x %8x %8d %8d %8d %8d %8d %8d %8.1f %8.3f %16s %16s'
            for dpid in bodys.keys():
                for stat in sorted(bodys[dpid], key=attrgetter('port_no')):
                    if stat.port_no != ofproto_v1_3.OFPP_LOCAL:
                        print(format % (
                            ...
                            self._get_free_bw(CONF['band'],  abs(self.port_speed[(dpid, stat.port_no)][-1])),
 #self.port_features[dpid][stat.port_no][2],  abs(self.port_speed[(dpid, stat.port_no)][-1])),
